...
class InferencePipeline:
    ...
    def get_model_tokenizer_and_config(self):
        if self.model_name == ModelName.GPT_3_5.value or self.model_name == ModelName.GPT_4.value:
            ...
            print("doing nothing as using ollama")
            # self.tokenizer = AutoTokenizer.from_pretrained(self.checkpoint, trust_remote_code = True)
    ...
    def model_generate(self, prompt, prompt_test = None):
        if self.model_name == ModelName.GPT_3_5.value or self.model_name == ModelName.GPT_4.value:
            ...
            # outputs = ollama.generate(model="llama3.2:1b", prompt=prompt)['response']
            ...
            try:
                ...
                # testpred
                # print("calling groq1")
                # # ask groq to generate tests for the given spec
                ...
                # )
                # outputs = result.choices[0].message.content
                # # now ask groq to generate code for the given spec and add tests response in chat
                # print("calling groq2")
                # result = client1.chat.completions.create(
                #     model=GROQ_MODEL,
                #     messages=[
                #         {"role": "user", "content": prompt_test},
                ...
                # testpred
                # print("calling google1")
                # # ask google to generate tests for the given spec
                # chat = client.chats.create(model=GOOGLE_MODEL)
                # response = chat.send_message(prompt_test)
                # # now ask google to generate code for the given spec and add tests response in chat
                # print("calling google2")
                # response = chat.send_message(prompt)
                # outputs = response.text
    ...
    def construct_prompt_test(self, strategy, info):
        prompt = ""
        class_name = info['class_name']
        skeleton = info['skeleton']
        # ask the model to generate tests for the given spec
        instruction = f"Please generate tests for the class {class_name} in the following code."
    ...
    def pipeline(self):
        ...
        if self.generation_strategy == GenerationStrategy.Holistic.value:
            ...
            for cont in tqdm(self.file_cont):
                # skip the first 47 tasks format: task_id is like ClassEval_X
                # if int(cont['task_id'].split("_")[1]) <= 96:
                #     print("Skipping task_id: ", cont['task_id'])
                #     continue
                ...
                try:
                    ...
                    for _ in range(self.SAMPLE_NUMS):
                        outputs = self.model_generate(prompt, self.construct_prompt_test(GenerationStrategy.Holistic, cont))
                        pred.append(outputs)
                    cont['predict'] = pred
                    result.append(cont)
